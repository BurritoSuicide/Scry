use super::{HashKind, Indicator, IndicatorType};
...
fn ipv6_re() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    // Practical (not exhaustive) IPv6 matcher for OSINT input lists.
    RE.get_or_init(|| {
...
fn ja4_re() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    // JA4 / JA4H style: letter + digits + optional letter, then underscore-separated hex blocks.
    // Examples: t13d1516h2_8daaf6152771_b186095e22b6
    RE.get_or_init(|| {
...
pub fn detect_file(path: &Path) -> Result<DetectionSummary> {
    let contents = fs::read_to_string(path).map_err(|e| {
        CharonError::msg(format!("failed to read {}: {e}", path.display()))
    })?;
    ...
    if summary.indicators.is_empty() {
        return Err(CharonError::msg("input file contained no indicators"));
    }
